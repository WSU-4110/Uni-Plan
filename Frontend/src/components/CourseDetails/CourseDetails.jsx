export default function CourseDetails({ course, onClose }) {
    ...
    return (
        <div
            className="bg-white rounded-lg max-w-2xl w-full relative mx-auto overflow-hidden"
            style={{ maxHeight: 'calc(100vh - 120px)' }}
        >
            <button
                onClick={onClose}
                aria-label="Close course details"
                className="absolute top-3 right-3 z-10 text-gray-600 hover:text-gray-900 text-2xl leading-none"
            >
                ×
            </button>
